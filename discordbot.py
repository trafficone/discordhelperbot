...
def search_boardgame(boardgame):
    ...
    resp = ""
    if len(games) == 0:
        resp += "Couldn't find boardgame %s exactly, but I found %d which were close\n"%(boardgame,len(res))
        games = [x for x in res if boardgame.lower() in x.name.lower()]
        if len(games) == 0:
            resp += "Well, not exactly close, but here's the oldest one on the list\n"
            games = res
    g = sorted(games , key=lambda x:x.year)[0] 
    url_format = "https://www.boardgamegeek.com/boardgame/{id}/{name}".format(id=g.id,name=g.name.lower().replace(' ','-')) 
